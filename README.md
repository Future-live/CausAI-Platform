...
- Pandas / NumPy：数据读取、清洗、统计和图表数据准备。
...
- `services`：承载业务逻辑，包括 CSV 上传、版本派生、统计计算、图表数据生成、因果任务运行和大模型调用。
...
- `datasets`：原始 CSV 数据集、所属用户、存储路径、行列数和字段信息。
...
- 上传文件使用安全文件名和 UUID 存储，限制 CSV 类型。
...
- `/data-upload.html`：CSV 上传与历史数据集选择。
